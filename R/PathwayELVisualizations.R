...
#' Generates visualizations for Pathway expression levels in each given sample
#'
#' @param output_dir Directory to save visualizations to. If unspecified, visualizations will not be saved and instead will simply be printed out.
#' @param fileList A list of all the "pathway-expression-levels" CSV files generated by either RunSingleAnalysis() or RunMultiAnalysis().
#'
#' @export
PathwayVisualizations <- function(output_dir = NULL, fileList) {
  # Initialize global max variables
  global_ligand_max <- 0
  global_receptor_max <- 0
  global_reporter_max <- 0
  ...
  for(curr_file_path in fileList) {
    expressionLevels <- read.csv(curr_file_path, check.names = FALSE)
    celltype_columns <- setdiff(names(expressionLevels), c('Pathway', 'sample', 'gene', 'gene_type'))
    sample_name <- sub(".*/pathway-expression-levels_(.*)\\.csv", "\\1", curr_file_path)
